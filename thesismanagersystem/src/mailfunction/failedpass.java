...
public class failedpass extends Thread {
	...
	public void run() {
	 ...
	 public Message createEmail(Session session,mailinfo info) throws Exception{
		          ...
		          String infomation = "选题未被通过，请选择其他的论文祝你成功"+ ",你所选的论文题目：" + info.getThesistitle() + "，指导老师："+info.getThesisteacher()+"指导老师职称："+info.getThesistitle()+",老师qq："+info.getQqnumber()+"，老师邮箱："+info.getTomail()+"，老师手机号码："+info.getTelephone()+",论文简介："+info.getBrief();
		          message.setContent(infomation, "text/html;charset=UTF-8");
		          message.saveChanges();
		          return message;
		      }

public  failedpass(mailinfo info) {
	this.info=info;
}
